...
**决定**：补 `不太 / 不很 / 还算 / 有点 / 略微` 等 tier-1 词。
...
**决定**：改为限定作用域——不检查选项标签与量表锚点；题目属于**已声明构念的
量表电池**时降级或不报（特质题的绝对化措辞是量表惯例）；单独成题的
满意度/是否类问题维持 warning。若作用域判定成本过高，则整条降为 info，
但**必须保留检出能力**。
...
**决定**：`Option` 增加 `residual: bool`，计数时排除。
**并记一笔**：残差类别在**分析阶段**同样要排除——频次分布的分母、
卡方检验的列联表都不应把「其他 / 不适用 / 拒答」当作实质类别。
这一点须在批 3 的分析计划里落实，不只是校验器的计数问题。
