...
## 11. 维护约定：两份 README 必须同步

`README.md` 与 `README.zh-CN.md` 是**两个入口，不是主从关系**。
中文读者落地在 `README.zh-CN.md`，看到什么就是他对项目的全部第一印象。

**任何一份的结构性改动都必须同步到另一份**，至少包括：

| 项目 | 说明 |
| --- | --- |
| badge | tests / license 徽章，含 URL 参数（如是否钉分支） |
| 目录结构树 | 新增模块必须两边都补 |
| 快速开始 | 安装、启动命令、端口 |
| 功能清单 | 新增能力的描述 |
| 外链 | live demo、文档链接 |

**这条约定的由来**：2026-08-01 合并 PR #5 时发现，`README.md` 的结构树
已在前一轮修过，`README.zh-CN.md` 却仍停在四目录 MVP，**且完全没有 badge**——
中文读者看到的是一个"半年前的小项目"，而英文读者看到的是真实状态。
两份文档因为分开修而漂移，是这类问题的默认结局，除非写成约定。

**不需要同步的**：正文措辞、示例、面向不同语言读者的补充说明。
同步的是**结构与事实**，不是逐句翻译。
