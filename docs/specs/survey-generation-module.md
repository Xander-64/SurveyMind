...
## 18. 批 3 交付记录：analysis_plan.py（2026-08-01）

### 18.1 落实的数字

| 场景 | 权威值（nct 精确搜索） | 闭式近似 | 闭式值的实际功效 |
| --- | ---: | ---: | ---: |
| 两组均值 | **64** | 63 | 0.7952 ✗ |
| ANOVA k=3 → pairwise+Bonferroni | **86** | 84 | 0.7934 ✗ |
| ANOVA k=4 | **99** | 97 | 0.7917 ✗ |
| ANOVA k=5 | **109** | 107 | 0.7928 ✗ |
| 比例估计 | **385** | — | — |

闭式值**保留在输出里**（`closed_form_n` + `closed_form_actual_power`），
让"为什么不用它"这件事自明，而不是只留一个结论。

测试不只断言这些数字，还独立验证了两条性质：
**每个 `min_n` 都达到它声称的功效**，且 **`min_n − 1` 达不到**（最小性）。

### 18.2 信度

α 一律带 **Feldt 95% CI**（除法形式）+ **校正后的题项-总分相关**。

测试用三条断言把 Feldt 的形式钉死：等于除法形式；等于 **df 交换后**的乘法形式；
**不等于** df 未交换的乘法形式。最后一条是关键——写错 df 顺序不会报错，
只会给出一个看起来合理的、偏移的区间。

caveat 写明 τ 等价假设、题项方差不齐时区间偏窄、
以及"题项-总分相关 < 0.30 是该假设已被破坏的信号"。

### 18.3 一个被信度计算暴露出来的真实缺陷 ★

首次跑 α 得到 **−0.08**，全部题项-总分相关约等于 0。

原因：`synthetic.py` 的 `_scale_values` **注释写着**「给每个受访者一个潜在倾向
使同构念题项相关」，**但代码里没有实现**——每道题独立抽样。
注释描述的是意图，代码是另一回事。

修复：真正实现潜变量（每受访者每构念一个标准化潜分，题项 = 潜分 + 题目噪声，
`item_noise=0.55`）。修复后 α = 0.68 / 0.67，题项-总分相关 0.38–0.57，
**落在真实区间而非天花板**——这样信度输出才有阅读价值。

**这个缺陷是被"计算一个真实统计量"逼出来的，不是被读代码发现的。**
批 2 的测试全绿却没发现它，因为那些测试没有任何一处需要题项之间真的相关。

### 18.4 顺带修正批 2 的一处过度声称

批 2 我报告过「反向计分题重编码后与同构念其他题相关性转正（> 0.5）」，
并称这是反向计分不可恢复的证据。**这条断言当时偏弱**：它比较的是
**构念得分**与那三题的均值，而构念得分本身就含那三题，
相关高有很大一部分来自成分重叠——潜变量还没实现时它也能通过。

已改为干净的检验：**单独取重编码后的那一道题**与其他三题比较。
